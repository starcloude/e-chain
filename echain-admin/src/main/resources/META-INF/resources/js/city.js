...
function initCitySelect(config,$form) {
    config.v1 = config.v1 ? config.v1 : "";
    config.v2 = config.v2 ? config.v2 : "";
    $.each(provinceMap, function (k, v) {
        appendOptionTo($form.find('select[name=' + config.s1 + ']'), k, k, config.v1);
    });
    //默认值
    cityEvent({value:config.v1});
    layui.form.render();
    //绑定事件
    layui.form.on('select(' + config.s1 + ')', function (data) {
        cityEvent(data);
    });

    function cityEvent(data) {
    	var ele = $form.find('select[name=' + config.s2 + ']');
        ele.html("<option value=''>-请选择(市)-</option>");
        $.each(provinceMap[data.value], function (k, v) {
            appendOptionTo(ele, v, v, config.v2);
        });
        layui.form.render();
    }
